...
def load_data():
    ...
    # If none matched, pick ANY csv under /data and tell the user which one
    any_csvs = glob.glob("data/**/*.csv", recursive=True)
    if any_csvs:
        st.info(f"Using detected file: {any_csvs[0]}")
        return pd.read_csv(any_csvs[0])

    st.error("CSV not found. Please put your dataset inside a `data/` folder in the repo.")
    st.stop()

df = load_data()

# ---- Debug Info (optional, can delete after fixing) ----
...
# Rename columns to simple names (adjust if yours differ)
